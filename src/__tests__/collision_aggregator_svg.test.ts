...
{
    const html =
    '<svg id="svg" width="500" height="600" xmlns="http://www.w3.org/2000/svg" stroke-width="5px">' +
    ...
    const aggregatedShapes = createHtmlAndParse(html);

    expect(aggregatedShapes.length).toBe(9);
});
...
const createHtmlAndParse = (html: string): Command[][] =>
{
    ...
    const commands = pathArray
        .map(path =>
        {
            ...
            return expandCommands(absoluteCommands);
        });
